...
namespace m19 {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    int _lbl, _body_lbl, _sections_lbl;
    bool _inFunctionBody, _inFunctionArgs, _inFinalSection;
    ...
    std::stack<int> _break, _continue;

  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<m19::symbol> &symtab,
                   cdk::basic_postfix_emitter &pf) :
    basic_ast_visitor(compiler), _symtab(symtab), _pf(pf), _lbl(0),
      _body_lbl(0), _sections_lbl(0), _inFunctionBody(false), _inFunctionArgs(false), _inFinalSection(false) {
    }
